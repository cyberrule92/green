...
| `workflows.py` | Carbon-aware workflow automation engine (n8n / Make style). Dependency-free + import-cycle-free like `coding_agent`: heavy capabilities (CSS-routed LLM, RAG, guardrails, coding agent, image gen, grid CI) are injected as callables via `WorkflowServices` from `decision_engine`. A workflow is a DAG of typed nodes — **triggers** (`manual`/`webhook`/`schedule`/`carbon_window`/`error_trigger`), **AI actions** (`llm`/`rag_query`/`agent_task`/`guardrail`/`image_gen`, all CSS-routed and carbon-metered), **flow logic** (`if`/`switch` multi-way/`filter`/`wait`/`set_variables`/`carbon_gate`/`merge`), **io** (`http_request` with optional stored credential, `notify` webhook/email). Async engine walks the graph in dependency order; branch nodes gate downstream edges via `_active_handles`; data flows downstream via `{{ $node.<id>.field }}` / `{{ $input }}` / `{{ $trigger }}` / `{{ $vars.* }}` templates (`$vars` is one run-level dict shared by reference, written by `set_variables` and carried through an approval pause). Every run accumulates a real gCO2 receipt. **Prod-grade orchestration**: parallel **superstep** execution (independent nodes in a superstep run concurrently under `WF_MAX_PARALLEL`, LangGraph-style), per-node `retries`/`retry_backoff_s`/`timeout_s`/`on_error` (`stop`\|`continue`), a `subworkflow` node that runs another saved workflow (LangGraph subgraph parity) with optional `items` fan-out (map/batch), and an `approval` node for **human-in-the-loop** (LangGraph interrupt/resume parity): reaching it pauses the run, snapshots JSON-serialisable engine state to `workflow_runs.engine_state_json` with status `awaiting_approval`, and `POST /api/workflows/runs/{run_id}/approve` (`{node_id, approved, note?}`) resumes down the approved/rejected branch. Edges are keyed by stable index so state survives the pause. Autonomous **scheduler** in `decision_engine` startup (`_workflow_scheduler_loop`) fires `schedule` (cron via `cron_matches`) and `carbon_window` triggers — no external cron needed (`WF_SCHEDULER_ENABLED`, `WF_SCHEDULER_INTERVAL_S`, `WF_CARBON_WINDOW_DEBOUNCE_S`). SQLite tables `workflows` + `workflow_runs` + `workflow_credentials` in `green_ai.db`. **Credentials**: HTTP auth secrets encrypted at rest by a stdlib secret box (HMAC keystream + encrypt-then-MAC; `WF_SECRET_KEY`, falling back to the audit key), resolved **only** with the running workflow's tenant — the node's URL is author-controlled, so an unscoped lookup would exfiltrate another tenant's token — injected into request headers at dispatch and never present in `node_states` or the templating scope. **Cancellation**: an `asyncio.Event` per run checked between supersteps, so in-flight nodes finish and the run ends `cancelled`. **Failure handlers**: `graph.settings.on_error_workflow_id` runs on `failed`/`cancelled` with `{workflow_id, run_id, error, failed_node}`, bounded by `WF_MAX_ERROR_DEPTH` so a cyclic handler terminates. Per-node `duration_ms`/`started_at`/`finished_at` land in `node_states`. Endpoints under `/api/workflows` (CRUD, `/{id}/run`, `/{id}/runs`, `/runs/{run_id}`, `/runs/{run_id}/approve`, `/runs/{run_id}/receipt`, `/runs/{run_id}/cancel`, `/credentials`, `/node-types`, `/webhook/{id}`, and `/templates` + `/templates/{id}` + `/templates/{id}/instantiate` — declared *before* `/{workflow_id}` so "templates" isn't parsed as an id). **Template gallery**: `workflow_templates.py` seeds 77 runnable, validated templates across 9 industries (auto-laid-out; each carries `scenario`/`carbon`/`analog` prose the docx generator consumes); instantiate copies a template's graph into a new **disabled** workflow. Frontend "Browse templates" modal groups them by industry. Run `python3 workflows.py` for the stub-services smoke test (14 cases: branch/parallel/retry/subworkflow/switch/filter/wait/$vars/cancel/secret box/credential tenant isolation). `WF_MAX_NODES`/`WF_MAX_STEPS`/`WF_MAX_DEPTH`/`WF_HTTP_TIMEOUT_S` cap runaway graphs |
