...
class _VariantModel:
    """Two linear heads (accuracy residual, latency-scale residual) for one variant."""

    __slots__ = ("w_acc", "w_lat", "n_obs")

    def __init__(self, w_acc: list[float] | None = None, w_lat: list[float] | None = None, n_obs: int = 0):
    ...
    def predict(self, x: list[float]) -> tuple[float, float]:
        """Return (accuracy_residual, latency_scale)."""
        acc_resid = _dot(self.w_acc, x)
        acc_resid = max(-ACC_RESIDUAL_CLAMP, min(ACC_RESIDUAL_CLAMP, acc_resid))
        # Latency head predicts a residual around scale 1.0.
        lat_scale = 1.0 + _dot(self.w_lat, x)
        lat_scale = max(LAT_SCALE_MIN, min(LAT_SCALE_MAX, lat_scale))
        return acc_resid, lat_scale

    def update(self, x: list[float], acc_target_resid: float, lat_target_resid: float) -> None:
        """One clamped SGD step with L2 for each head."""
        acc_pred = _dot(self.w_acc, x)
        acc_err = acc_pred - acc_target_resid
        lat_pred = _dot(self.w_lat, x)
        lat_err = lat_pred - lat_target_resid
    ...
    def from_dict(cls, d: dict[str, Any]) -> "_VariantModel":
        ...
        return cls(_fit(d.get("w_acc")), _fit(d.get("w_lat")), int(d.get("n_obs", 0)))
...
class QualityLatencyEstimator:
    ...
    def adjust(
        ...
        """
        Return adjusted accuracy/latency for one candidate variant.
        ...
        try:
            ...
            acc_resid, lat_scale = model.predict(x)
            adj_acc = max(0.0, min(1.0, baseline_accuracy + acc_resid))
            adj_lat = max(1.0, baseline_latency_ms * lat_scale)
            result.update({
                "accuracy": adj_acc,
                "latency_ms": adj_lat,
    ...
    def observe(
        ...
        try:
            ...
            with self._lock:
                ...
                model.update(x, acc_target_resid, lat_target_resid)
                self._dirty = True
                self._updates += 1
