...
state — config/model_zoo.json, config/policies.json and data/rl_state.json.
Nothing is illustrative unless it says so on the chart itself.
...
def diagram_hld() -> Path:
    ...
    _box(ax, 0.06, 0.885, 0.88, 0.075, "React 19 + Grommet SPA  ·  green-frontend (nginx :8080)",
         sub="Chat · Carbon · Observability · Models   —   src/lib/api.js centralises every fetch",
...
def diagram_workflow() -> Path:
    ...
    _panel(0.58, 0.365, 0.38, 0.195, "deferred_queue.py", [
        ...
        ("training jobs are its first real caller", 7.3, AMBER, "i"),
    ], "#FDF6EC", AMBER)
...
def chart_deferral() -> Path:
    ...
    # set so the peak actually crosses the deferral threshold — the previous curve
    # topped out at it, so nothing in the picture would ever have been deferred, and
    ...
    submit_h = 9
    deadline_h = submit_h + 6                       # bounded wait: 6 h
    ...
    ax.text(47.6, defer_ci + 8, f"deferral threshold = {defer_ci}",
            fontsize=7.2, color=RED, va="bottom", ha="right")

    ax.scatter([submit_h], [ci[submit_h]], s=90, color=RED, zorder=6,
               edgecolor="white", lw=1.2)
    ax.annotate("job submitted — grid is above the\nthreshold, so the job is QUEUED, not run",
    ...
    ax.annotate("the global minimum is outside the deadline.\nDeferral is bounded: it takes the best window it\n"
                "can reach, and expiry still runs the job.",
    ...
    ax.set_xlim(-1, 49)
    ax.set_ylim(240, 780)
    ax.set_title("EcoServe deferral — training jobs wait for the greenest reachable "
                 "window (chat's deferral is advisory only)", fontsize=9.5, color=INK, pad=30)
...
def build():
    ...
    E.append(Paragraph(
        "Routing, deferral, retrieval and safety —<br/>"
    ...
    E.append(table([
        ...
        [cell("vllm-stem-coding"), cell("8006"), cell("Qwen2.5-Coder-1.5B — the greenest <i>code-capable</i> model, and the single backend for every coding request.")],
    ...
    E.append(P(
        ...
        "status read for its whole duration; a long-running dispatch therefore spawns a worker thread and returns "
    ...
    # ------------------------------------------------------------------ 6. LLD
    E.append(P("6 · Low-level design", "h1"))
    E.append(P("6.1 Module responsibilities", "h2"))
    E.append(table([
    ...
    E.append(PageBreak())
    E.append(P("6.2 The candidate registry", "h2"))
    ...
    E.append(P("6.3 Failure is designed, not discovered", "h2"))
    E.append(P(
    ...
    E.append(table([
        ...
        [cell("vLLM backend"), cell("Rule-based extractive answer from retrieved evidence, so a wedged or missing engine degrades the answer rather than dropping the request.")],
    ...
    E.append(P("6.4 Evidence — the HMAC-signed audit trail", "h2"))
    E.append(P(
    ...
    E.append(table([
        ...
        [cell("<b>EcoServe deferral</b>"), cell("48-hour forecast, ≥15 % CI-drop threshold, deadline-bounded queue with back-pressure. Training jobs are its first real caller."), cell("Live")],
    ...
    E.append(P("7.1 Honest limits", "h2"))
    E.append(P(
    ...
    # ------------------------------------------------------------ 9. CLOSING
    E.append(P("8 · Why the design holds together", "h1"))
    E.append(table([
    ...
    E.append(P(
        "The measured result: carbon is a first-class term in every routing decision, and the books that "
        "prove it are the same signed rows the router wrote while making them.", "pull"))
    E.append(Spacer(1, 4 * mm))
    E.append(P(
        ...
        "RL curve comes from data/rl_state.json on this deployment. Where a chart is illustrative rather "
        "than measured, it says so on the chart.", "body"))
